...
const db = () => JSON.parse(localStorage.getItem(storageKey) || JSON.stringify(createSeed()));
const save = (data) => localStorage.setItem(storageKey, JSON.stringify(data));
const ok = (data) => HttpResponse.json({ code: 0, message: 'success', data });
const page = (items, url) => {
    const keyword = url.searchParams.get('keyword') || '';
    const status = url.searchParams.get('status') || '';
    const filtered = items.filter((item) => (!keyword || JSON.stringify(item).includes(keyword)) && (!status || item.status === status));
...
export const handlers = [
    ...
    http.get('/api/admin/dashboard', () => {
        ...
        return ok({
            kpis: [
                ...
                { label: '待审核内容', value: data.contents.filter((item) => item.status === 'pending').length, trend: '需及时处理' },
            ],
    ...
    http.patch('/api/admin/users/:id/status', async ({ params, request }) => {
        ...
        user.status = (await request.json()).status;
    ...
    http.post('/api/admin/users/:id/credits', async ({ params, request }) => {
        const data = db();
        const user = data.users.find((item) => item.id === params.id);
        user.credits += (await request.json()).amount;
    ...
    http.patch('/api/admin/contents/:id/review', async ({ params, request }) => {
        ...
        content.status = (await request.json()).status;
